/**
 * Parsing model structures into JSON-like format and restore them later on.
 * We'll need a special format to capture the Model-aspects of a Collection.
 * Note that for this to work, the build minifier must be configured with
 * some kind of `keep_classnames: true` setting!
 */
...
const iscoll = (thing) =>
	islist(thing) && constr(thing).isCollectionConstructor;
...
function remodel(proto) {
	return Object.entries(proto).filter(proper(proto)).reduce(subtree, {});
}
